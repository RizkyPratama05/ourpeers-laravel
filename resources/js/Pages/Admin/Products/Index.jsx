...
export default function Index({ auth, products, categories }) {
    ...
    return (
        <AuthenticatedLayout header="Katalog Produk">
            ...
            <div className="space-y-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    {[
                        ...
                        <div
                            key={i}
                            className={`${stat.color} p-8 rounded-[3rem] shadow-sm border border-slate-100/10 flex items-center gap-6 group hover:shadow-xl transition-all duration-500`}
                        >
                            <div
                                className={`w-14 h-14 rounded-2xl flex items-center justify-center ${stat.text} bg-white/10 group-hover:scale-110 transition-transform`}
                            >
                                {stat.icon}
                            </div>
                            <div>
                                <p
                                    className={`text-[10px] font-black uppercase tracking-widest ${stat.text === "text-white" ? "text-white/60" : "text-slate-400"}`}
                                >
                                    {stat.label}
                                </p>
                                <p
                                    className={`text-3xl font-black ${stat.text === "text-white" ? "text-white" : "text-brand-navy"}`}
                                >
                                    {stat.value}
                                </p>
                            </div>
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 bg-white p-10 rounded-[3.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.02)] border border-slate-100 relative overflow-hidden group">
                    ...
                    <div className="relative">
                        ...
                        <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">
                            Kelola stok dan katalog penjualan Anda
                        </p>
                    </div>
                    <div className="flex items-center gap-3 relative z-10">
                        {/* Shortcut ke halaman kategori */}
                        <Link
                            href={route("admin.categories.index")}
                            className="bg-white border border-slate-200 text-brand-navy px-6 py-4 rounded-[1.5rem] flex items-center gap-2 hover:border-brand-lime transition-all duration-300 font-black uppercase text-xs tracking-widest"
                        >
                            <Tag size={16} /> Kategori
                        </Link>
                        <Link
                            href={route("admin.products.create")}
                            className="bg-brand-navy text-white px-10 py-5 rounded-[2rem] flex items-center gap-3 hover:bg-brand-lime hover:text-brand-navy transition-all duration-700 font-black uppercase text-xs tracking-widest shadow-[0_20px_40px_rgba(15,23,42,0.2)] hover:shadow-[0_20px_40px_rgba(132,204,22,0.3)] hover:-translate-y-1"
                        >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {products.map((product) => (
                        <div
                            key={product.id}
                            className="group bg-white rounded-[4rem] overflow-hidden shadow-[0_10px_40px_rgba(0,0,0,0.02)] border border-slate-50 hover:shadow-[0_40px_80px_rgba(0,0,0,0.08)] hover:-translate-y-3 transition-all duration-700"
                        >
                            <div className="h-64 bg-slate-100 relative overflow-hidden">
                                ...
                                <div className="absolute top-6 right-6 bg-white/95 backdrop-blur-md px-4 py-2 rounded-2xl shadow-xl">
                                    <div className="flex items-center gap-2">
                                        <div
                                            className={`w-1.5 h-1.5 rounded-full ${product.stok > 10 ? "bg-brand-lime" : "bg-red-500"} animate-pulse`}
                                        ></div>
                                ...
                                {product.category && (
                                    <div className="absolute top-6 left-6 bg-brand-lime text-brand-navy px-3 py-1 rounded-xl text-[10px] font-black uppercase tracking-widest shadow">
                            ...
                            <div className="p-10 relative">
                                ...
                                <div className="mt-8">
                                    <h4 className="text-xl font-black text-brand-navy mb-2 tracking-tight group-hover:text-brand-lime transition-colors duration-300 uppercase">
                                        {product.nama_produk}
                                    </h4>
