...
export default function AuthenticatedLayout({ header, children }) {
    ...
    return (
        <div className="min-h-screen bg-[#F1F5F9] flex font-sans selection:bg-brand-lime selection:text-brand-navy relative overflow-hidden">
            ...
            >
                <div className="p-8 flex items-center justify-between">
                    {isSidebarOpen ? (
                        <Link
                            href="/"
                            className="font-black text-3xl tracking-tighter flex items-center group"
                        >
                            OURPEERS
                            <span className="text-brand-lime transition-transform group-hover:scale-125 duration-300 ml-1">
                                .
                            </span>
                        </Link>
                    ) : (
                        <div className="w-10 h-10 bg-brand-lime rounded-xl flex items-center justify-center mx-auto">
                            <span className="font-black text-brand-navy text-xl">
                                O
                            </span>
                        </div>
                    )}
                ...
                <div className="px-6 py-4">
                    ...
                    >
                        {isSidebarOpen ? (
                            ...
                            <Menu size={18} className="text-brand-lime" />
                        )}
                    </button>
                </div>
                ...
                <nav className="flex-grow px-6 py-8 space-y-3">
                    {navItems.map((item) => (
                        ...
                        >
                            <span
                                className={`${item.active ? "text-brand-navy" : "text-slate-500 group-hover:text-brand-lime"} transition-colors duration-300`}
                            >
                                {item.icon}
                            </span>
                            {isSidebarOpen && (
                ...
                <div className="p-6 mt-auto border-t border-white/5 space-y-3">
                    ...
                    >
                        <Home
                            size={18}
                            className="group-hover:text-brand-lime transition-colors"
                        />
                        {isSidebarOpen && (
            ...
            {/* Main Content */}
            <main
                className={`flex-grow transition-all duration-700 ease-[cubic-bezier(0.34,1.56,0.64,1)] ${isSidebarOpen ? "ml-80" : "ml-24"} p-10 relative z-10`}
            >
                {header && (
                    <header className="mb-12 flex justify-between items-center bg-white/70 backdrop-blur-2xl p-10 rounded-[3.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.02)] border border-white/50 ring-1 ring-black/[0.02]">
                        <div className="space-y-2">
                            ...
                            <div className="text-4xl font-black text-brand-navy tracking-tighter uppercase">
                                {header}
                            </div>
                        </div>
                        <div className="flex items-center gap-6">
                            <div className="text-right hidden lg:block">
                                <p className="text-sm font-black text-brand-navy tracking-tight uppercase">
                                    {user.name}
                                </p>
